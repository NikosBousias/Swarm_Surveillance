...
%             plot3_poly( [sim.Cd{i} ; zeros(size(sim.Cd{i}(1,:)))], 'c--');
            if ~isempty(sim.W{i}) & i~=6
                %solve nan case for disjoint W 
        ...
        for i=1:sim.N
            ...
            %quadrotor_plot(fig, [sim.X(i), sim.Y(i), sim.Z(i)], [sim.phi(i), sim.theta(i), sim.psi(i)] );
            plot3( [sim.X(i) sim.X(i)], [sim.Y(i) sim.Y(i)], [sim.Z(i) 0], 'k--.' )
            if length(sim.W{i})>0
                for j=1:24:length(sim.W{i}(1,:))
                    plot3([sim.W{i}(1,j) sim.X(i)], [sim.W{i}(2,j) sim.Y(i)], [0 sim.Z(i)], 'r--');
                end
            end
        end
