...
%         
        %Manual Construction
        Rz = [cos(psi) -sin(psi) 0;sin(psi) cos(psi) 0;0 0 1];   %Rotation mappings
        Rx = [cos(the) 0 sin(the);0 1 0;-sin(the) 0 cos(the)];
        Ry= [1 0 0;0 cos(phi) -sin(phi);0 sin(phi) cos(phi)];
        R = Ry*Rx*Rz;    %Rotation matrix
